...
class BaseREParser(object):
    ...
    def __init__(self, handler_object):
        ...
        self.py_string_escape = SPEC_CHAR_PYTHON_STRING.copy().setParseAction(self.handler_object.handleStringEscape)
        end_range = (COLL_ELEM ^ collating_symbol
            ^ self.py_string_escape) # added by me
        ...
        single_expression = (end_range ^ character_class ^ equivalence_class ^
                                self.py_string_escape ) # added by me
        expression_term = single_expression ^ range_expression.setParseAction(self.handler_object.handleRange)
